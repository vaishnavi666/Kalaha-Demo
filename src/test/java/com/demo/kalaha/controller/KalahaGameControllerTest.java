...
/**
 * Integration tests for the Kalaha game Controller.
 */
...
public class KalahaGameControllerTest {
    ...
    /**
     * Test the initialization of a new Kalaha game.
     */
    @Test
    @DirtiesContext
    public void shouldInitGame() throws Exception {
        ...
        mockMvc.perform(initGameRequest)
                ...
                //check player index
                .andExpect(MockMvcResultMatchers.jsonPath("$.player1.playerIndex").value(KalahaGameControllerTest.PLAYER1_INDEX))
                .andExpect(MockMvcResultMatchers.jsonPath("$.player2.playerIndex").value(KalahaGameControllerTest.PLAYER2_INDEX))

                //check pit owner
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.6.playerIndex").value(KalahaGameControllerTest.PLAYER1_INDEX))
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.13.playerIndex").value(KalahaGameControllerTest.PLAYER2_INDEX))
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.14.playerIndex").value(KalahaGameControllerTest.PLAYER2_INDEX))

                //check  initial pit stone count
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.5.stoneCount").value(KalahaGameControllerTest.INITIAL_STONE_ON_PIT))
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.12.stoneCount").value(KalahaGameControllerTest.INITIAL_STONE_ON_PIT))
                .andExpect(MockMvcResultMatchers.jsonPath("$.board.pits.14.stoneCount").value(KalahaGameControllerTest.INITIAL_STONE_ON_HOUSE))

                .andReturn();
    }

    /**
     * Test playing the Kalaha game.
     */
    @Test
    @DirtiesContext
